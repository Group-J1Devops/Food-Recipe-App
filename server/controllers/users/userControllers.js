  ...
  try {
    ...
    if (existingUser) {
      ...
      if (!isMatch) {
        ...
        res
          .status(200)
          .json({ result, message: "User logged in successfully!" });
      }
    } else {
      return res
        .status(400)
...
exports.forgotpassword = async (req, res) => {
  ...
  try {
    ...
    if (!userFind) {
      res
        .status(404)
        .json({ errorMessage: "User not found, please check your email" });
    } else {
      ...
      if (setUserToken) {
        const mailOptions = {
          ...
          html: `
            ...
            <a href="http://localhost:3001/resetpassword/${userFind.id}/${setUserToken.verifyToken}" style="display: inline-block; padding: 10px 20px; margin: 20px 0; font-size: 13px; color: white; background-color:rgb(207, 91, 56); text-decoration: none; border-radius: 5px;">Reset Password</a>
        ...
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            ...
            res.status(200).json({
              message:
                "An email to reset your password has been sent successfully",
            });
          }
        });
      } else {
